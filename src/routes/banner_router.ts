...
const readValue = (input: Record<string, unknown> | FormData, names: string[]) => {
...
const readImageFileId = (input: Record<string, unknown> | FormData) => {
...
const getFormImage = (form: FormData) =>
  [
    ...form.getAll("image"),
    ...form.getAll("bannerImage"),
    ...form.getAll("banner_image"),
    ...form.getAll("file"),
  ].find((item): item is File => item instanceof File && item.size > 0) ?? null;
...
router.get("/", async (c) => {
  try {
    ...
    const where = [
      ...
      q
        ? or(
            ilike(tBanner.title, `%${q}%`),
            ilike(tBanner.subtitle, `%${q}%`),
            ilike(tBanner.description, `%${q}%`)
          )
        : undefined,
    ].filter(Boolean);
...
router.post("/", async (c) => {
  try {
    ...
    const existing =
      id > 0
        ? (
            await db.select().from(tBanner).where(eq(tBanner.id, id)).limit(1)
          )[0]
        : null;
    ...
    const uploadedImage =
      input instanceof FormData && getFormImage(input)
        ? await uploadCourseImage(getFormImage(input) as File, "banners/images")
        : null;
    const explicitImageFileId = readImageFileId(input);
    const imageFileId =
      uploadedImage?.id ?? explicitImageFileId ?? existing?.imageFileId ?? null;
    const now = new Date().toISOString();
    const payload = {
      ...
      linkTarget: readString(
        input,
        ["linkTarget", "link_target"],
        existing?.linkTarget ?? "_self"
      ),
      position: readString(input, ["position"], existing?.position ?? "main") || "main",
      isVisible: isTruthy(
        readString(input, ["isVisible", "is_visible"], String(existing?.isVisible ?? true)),
    ...
    const saved = (
      await db
        ...
        .set({
          ...
          linkUrl: readString(input, ["linkUrl", "link_url"], existing.linkUrl ?? ""),
          linkTarget:
            readString(input, ["linkTarget", "link_target"], existing.linkTarget ?? "_self") ||
            "_self",
          position: readString(input, ["position"], existing.position ?? "main") || "main",
          isVisible: isTruthy(
  ...
  try {
    ...
    const uploadedImage =
      input instanceof FormData && getFormImage(input)
        ...
        : null;
    const explicitImageFileId = readImageFileId(input);
    const updateData: Partial<typeof tBanner.$inferInsert> = {
      updatedBy: admin.id,
      updatedAt: new Date().toISOString(),
    };
