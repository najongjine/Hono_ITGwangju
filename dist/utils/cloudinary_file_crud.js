...
export const normalizeCloudinaryKey = (key) => key
    .replace(/\\/g, "/")
    .replace(/^\/+|\/+$/g, "")
    .split("/")
    .filter(Boolean)
    .join("/");
const withoutExtension = (key) => {
    const ext = path.posix.extname(key);
    return ext ? key.slice(0, -ext.length) : key;
};
export const uploadCloudinaryFile = async ({ key, body, contentType = "application/octet-stream", }) => {
    configureCloudinary();
...
export const deleteCloudinaryFile = async (key, resourceType = "image") => {
    configureCloudinary();
    const normalizedKey = normalizeCloudinaryKey(key);
