...
const readOptionalString = (input, names) => {
    for (const name of names) {
...
const normalizeEnrollmentStatus = (value, fallback = "pending") => {
...
router.get("/register/settings", async (c) => {
    try {
        return c.json(ok({ cloudflareTrustile: await isCloudflareTrustileRequired() }));
    }
    catch (error) {
    ...
    try {
        ...
        if (courseId || sessionId) {
            const sessionRows = await db
                ...
                .where(and(eq(tCourseSessions.id, nextSessionId), eq(tCourseSessions.courseId, nextCourseId)))
                .limit(1);
            if (!sessionRows[0]) {
