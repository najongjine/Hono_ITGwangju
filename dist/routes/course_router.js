...
import { tCourseSessions, tCourses, tFileLinks, tFiles, } from "../db/schema.js";
import { insertLocalFileMeta } from "./file_router_query.js";
import { uploadLocalFile } from "../utils/local_file_crud.js";
import { createSignedDownloadUrl, getStorageBucket, uploadFileAndCreateSignedUrl, } from "../utils/supabase_file_crud.js";
...
const getFormString = (form, names, fallback = "") => {
...
const getInputString = (input, names, fallback = "") => {
...
const uploadCourseImage = async (file, dir) => {
...
const withFileUrl = async (file) => {
    if (!file) {
        return null;
    }
    const storageKey = file.storageKey ?? "";
    const url = file.storageType === "supabase" && storageKey
...
const getCourseImages = async (course) => {
    ...
    return {
        ...
        descriptionImages: await Promise.all(descriptionRows.map(async ({ link, file }) => ({
            link,
...
const getCourseSessions = async (courseId, includeDeleted = false) => {
    const where = [
...
const getCourseSessionById = async (courseId, sessionId, includeDeleted = false) => {
    const where = [
        eq(tCourseSessions.courseId, courseId),
...
router.post("/:courseId/sessions", async (c) => {
    try {
        ...
        const payload = {
            ...
            applyStartDate: toNullableDate(getInputValue(input, ["applyStartDate", "apply_start_date"])),
            applyEndDate: toNullableDate(getInputValue(input, ["applyEndDate", "apply_end_date"])),
        ...
        const saved = await db.transaction(async (tx) => {
            ...
            if (uploadedDescriptionImages.length > 0) {
                await tx
                    .delete(tFileLinks)
                    .where(and(eq(tFileLinks.targetTable, COURSE_TABLE), eq(tFileLinks.targetId, course.id), eq(tFileLinks.fileRole, DESCRIPTION_IMAGE_ROLE)));
                await tx.insert(tFileLinks).values(uploadedDescriptionImages.map((file, index) => ({
                    fileId: file.id,
