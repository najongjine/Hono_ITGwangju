...
const NOTICE_STATUS_VALUES = ["published", "hidden"];
const normalizeNoticeStatus = (value, fallback = "published") => {
    const status = String(value ?? "").trim().toLowerCase();
    if (!status)
        return fallback;
    if (NOTICE_STATUS_VALUES.includes(status))
        return status;
...
const uploadNoticeImage = async (file, uploadedBy) => {
...
const getNoticeImages = async (noticeId) => {
    const rows = await db
        ...
        .where(and(eq(tFileLinks.targetTable, NOTICE_TABLE), eq(tFileLinks.targetId, noticeId), eq(tFileLinks.fileRole, NOTICE_IMAGE_ROLE), ne(tFiles.status, "deleted")))
...
const parseExistingImageIds = (input) => {
...
const saveNoticeImages = async (noticeId, files, existingImageIds, uploadedBy, imageOrders = []) => {
    ...
    await db.transaction(async (tx) => {
        await tx
            .delete(tFileLinks)
            .where(and(eq(tFileLinks.targetTable, NOTICE_TABLE), eq(tFileLinks.targetId, noticeId), eq(tFileLinks.fileRole, NOTICE_IMAGE_ROLE)));
        if (fileIds.length > 0) {
            await tx.insert(tFileLinks).values(fileIds.map((fileId, index) => ({
                fileId,
...
router.get("/", async (c) => {
    try {
        ...
        const data = await Promise.all(notices.map(async (notice) => ({
            ...notice,
...
router.post("/", async (c) => {
    try {
        ...
        const publishedAt = readString(input, ["publishedAt", "published_at"]) ||
...
router.put("/:id", async (c) => {
    try {
        ...
        const input = await getInput(c);
        const existing = (await db.select().from(tNotices).where(eq(tNotices.id, id)).limit(1))[0];
        if (!existing) {
            return c.json(fail(c, new Error("notice not found")));
        }
        ...
        const now = new Date().toISOString();
        const status = normalizeNoticeStatus(readString(input, ["status"], existing.status ?? "published"));
        const saved = (await db
            ...
            userId: admin.id,
            isPinned: isTruthy(readString(input, ["isPinned", "is_pinned"], String(existing.isPinned)), existing.isPinned ?? false),
            status,
            publishedAt: readString(input, ["publishedAt", "published_at"], existing.publishedAt ?? "") ||
...
router.delete("/:id", async (c) => {
    try {
        ...
        const deleted = await db.transaction(async (tx) => {
            const row = (await tx.delete(tNotices).where(eq(tNotices.id, id)).returning())[0];
            if (!row)
                return null;
            await tx
                .delete(tFileLinks)
                .where(and(eq(tFileLinks.targetTable, NOTICE_TABLE), eq(tFileLinks.targetId, id)));
            return row;
        });
        if (!deleted) {
            return c.json(fail(c, new Error("notice not found")));
