...
import { copyFile, createSignedDownloadUrl, createSignedUploadUrl, deleteFile, getFileInfo, getStorageBucket, listFiles, moveFile, uploadFileAndCreateSignedUrl, } from "../utils/supabase_file_crud.js";
...
const getDbFileByStorageKey = async (storageKey) => {
    const rows = await db
        .select()
        .from(tFiles)
...
router.get("/files", async (c) => {
    try {
        ...
        const dbFileMap = new Map(dbFiles.map((file) => [file.storageKey ?? "", file]));
        return c.json(ok(files.map((file) => ({
            ...file,
...
router.post("/files/signed-upload-url", async (c) => {
    try {
        ...
        if (body.contentType && isImageMimeType(body.contentType)) {
            return c.json(fail(new Error("image uploads must use POST /files so the server can convert them to webp")));
        }
